...
class StageRuntime:
    def __init__(self, model, distributed_backend, fp16, loss_scale,
                 ...
                 model_type, input_path, enable_recompute=False):
        ...
        subnets = json.load(open(input_path))
        self.ops = []
        self.partitions = []
        for sub in subnets:
            sub = json.loads(sub)
    ...
    def initialize(self, model, inputs_module_destinations,
        ...
        if True:#self.is_first_stage:
            batch_s = 192 
            rep = 2
            
            self.test_input_0 = [[104,999, 23, 44, 31, 41, 999, 50, 999, 6, 999, 999, 999, 4,  61,16, 14, 999, 999, 999,999, 7, 6, 999, 999,  4,248, 999, 9, 999, 5,  2]*rep]*batch_s

            self.test_input_1 = [[2, 104,333, 23, 44, 31, 41, 333, 50, 444, 6, 444, 555, 666, 4,  61,16, 14, 666, 666, 999,999, 7, 6, 999, 99,  4,99, 99, 9, 999, 5]*rep]*batch_s

            self.test_target = self.test_input_0
    @property
    def target(self):
    ...
    def set_loader(self, loader):
        if loader is not None:
            self.loader_iter = "performance"#loader.next_epoch_itr()
        else:
    ...
    def receive_tensors_forward(self): #, blocking = True):
        ...
        global test_input_0, test_input_1, test_target
        ### If current is empty:
        ...
        if self.loader_iter is not None:
            ...
            if self.model_type == TRANSLATION:
                ...
                #target = input['target']
                self.tensors[-1]["input0"] = torch.tensor(self.test_input_0, dtype=torch.int64, device=torch.device('cuda')) # inputs['src_tokens'].cuda(non_blocking=True)
                self.tensors[-1]["input1"] = torch.tensor(self.test_input_1, dtype=torch.int64, device=torch.device('cuda'))#inputs['prev_output_tokens'].cuda(non_blocking=True)
                self.tensors[-1]["target"] = torch.tensor(self.test_target, dtype=torch.int64, device=torch.device('cuda'))#target.cuda()
                self.tensors[-1]["ntokens"] = torch.zeros(input_t).cuda() # torch.zeros(input['ntokens']).cuda()

                # print("input size:")
                # print(self.tensors[-1]["input0"].size())
                
                # print(self.tensors[-1]["input0"].dtype)
                # print(self.tensors[-1]["input1"].size())
                # print(self.tensors[-1]["input1"].dtype)

                # print(self.tensors[-1]["target"].size())
                # print(self.tensors[-1]["target"])
                # print(input['ntokens'])
                
                # torch.randn(torch.Size([batch_s, seq_l]), dtype=torch.int64, device=torch.device('cuda'))
                # torch.Size([192, 17])
    ...
    def run_backward(self):
        ...
        for idx, t in enumerate(self.tensors):
            ...
            if t["seq"] == bwd_seq_id:
                ...
                #print("got bwd tensors seq", t["seq"])
                break
        #print("Backward: Stage ", self.stage)
