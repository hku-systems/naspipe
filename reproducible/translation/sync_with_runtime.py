...
def train(train_loader, r, optimizer, epoch, lr_scheduler):
    ...
    for t in range(total_updates):
        ...
        while True:
            ...
            while not r.receive_tensors_forward():
                continue
            while seq_id < 0 and len(r.buffer_list) < 30:
            ...
            while not r.receive_tensors_backward():
                continue
            if True:
            # perform backward pass
                ...
                if is_last_stage():
                    ...
                    full_epoch_time = (epoch_time / float(cnt)) * float(n)
                    ...
                    if cnt % args.print_freq == 0:
                        print('Stage: [{0}] Epoch: [{1}][{2}/{3}]\t'
                            ...
                            args.stage, epoch, cnt, n, timestamp=time.time(), batch_time=batch_time,
                    ...
                    if cnt % args.print_freq == 0:
                        print('Stage: [{0}] Epoch: [{1}][{2}/{3}]\tMemory: {memory:.3f} ({cached_memory:.3f})'.format(
                            args.stage, epoch, cnt, n, memory=(float(torch.cuda.memory_allocated()) / 10**9),
                            cached_memory=(float(torch.cuda.memory_cached()) / 10**9)))
                        import sys; sys.stdout.flush()
            #else:
