...
class Stage(nn.Module):
    ...
    def forward(self, input0, input1):
        ...
        #print("input 0", torch.sum(input0))
        #print("input 1", torch.sum(input1))
        return cp.checkpoint(self.forward_, input0, input1, self.dummy)
        #return self.forward_(input0, input1, self.dummy)

    def forward_(self, input0, input1, dummy=None):
        start = self.start
        #print("start", self.start, " forward with ", self.seq_id)
        if self.start == 0:
            assert self.end > 1
...
def model(criterion):
    model = TransformerModel.build_model(Args(), Task())
    module = transfer_model(model)
    for m in [module[0], module[1], module[-1]]: 
        #print(m) 
    ...
    for m in module[2:-1]:
        for layer in m.layer:
            for p in layer.parameters():
                ...
                p.name_ = "choices"
                #p.data = torch.empty(0)
            layer.register_forward_pre_hook(to_cuda_hook)
            layer.register_forward_hook(to_cpu_hook)
            # layer.register_backward_pre_hook(to_cuda_hook)
            # layer.register_backward_hook(to_cpu_hook)
        for p in m.layer_norm.parameters():
            p.step_ = False
            p.name_ = "layer_norm"
            p.type_ = "persist"
