...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, args, dictionary, embed_tokens, left_pad=False):
        ...
        if args.adaptive_softmax_cutoff is not None:
            ...
            #print("embed_out"+str(len(dictionary)) + " " + str(embed_dim))
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.layer_norm = FusedLayerNorm(args.encoder_embed_dim)
        self.idx = 0
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.layer_norm = FusedLayerNorm(args.decoder_embed_dim)
        self.idx = 0
...
def encoder_layer(args):
    ...
    for ks in [1, 3, 5]:
        for i in range(3):
            ops.append(
                opwrapper(StandardConv(embed_dim, embed_dim, ks))
            )
    # depthwise separable conv
    for ks in [3, 5, 7, 9, 11]:
        for i in range(3):
            ops.append(
                opwrapper(SeparableConv(embed_dim, embed_dim, ks))
            )
    # lightweight conv
    for ks in [3, 5, 7, 15]:
        for r in [1, 4, 16]:
            for i in range(3):
                ops.append(
                opwrapper(LightweightConv(ks, r))
                )
    # h head attention
    for hs in [4, 8, 16]:
        for i in range(3):
            ops.append(
                opwrapper(nn.MultiheadAttention(
                    embed_dim, hs,
    ...
    # gated linear
    for i in range(3):
        ops.append(opwrapper(GatedLinear(embed_dim, embed_dim)))
    # identity
    #ops.append(Identity())
