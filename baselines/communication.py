...
class CommunicationHandler(object):
    ...
    def setup_queues(self):
        ...
        for input_name in self.receive_ranks:
            #print("forward_receive_queues", input_name)
    ...
    def get_recv_queue(self,forward_minibatch_id = 0,
             backward_minibatch_id = 0, backward=False):
        if backward:
            index = (backward_minibatch_id + self.rank_in_stage) % \
                len(self.backward_receive_queues["control"])
            return self.backward_receive_queues["control"][
                index]
        else:
            index = self.get_messaging_index(sending=False)
            #print("Messaging index", index)
            return self.forward_receive_queues["control"][
                index]
    ...
    def recv(self, tensor_name, forward_minibatch_id,
        ...
        if backward:
            ...
            # if self.rank == 1 and tensor_name == "control":
            #     print(self.backward_receive_queues[tensor_name][
            #         index].queue[0][0])
...
def recv_helper_thread(queue, counter, local_rank, tensor_name,
    ...
    for i in range(num_iterations):
        ...
        queue.add(tensor)
        # if tensor_name == "control" and local_rank == 1 and src_rank == 2:
        #     print(queue)
        #     print("----------", tensor[0], "----------", queue.empty())
    counter.decrement()
